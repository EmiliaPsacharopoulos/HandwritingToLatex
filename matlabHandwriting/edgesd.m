...
Xo = load('Matlab/trainset.mat');
Yo = load('Matlab/trainsetLabels.mat');
% Mdl = fitctree(Xo.images,Yo.labels); %works
Mdl = fitcknn(Xo.images,Yo.labels,'NumNeighbors',5,'Standardize',1);
close all;
...
%             imshow(letter);
            ...
            let = imresize(let,[50 50]);
            let = let(:)';
            label = predict(Mdl, let);
